...
当前项目阶段：以 [`project-state.json`](../project-state.json) 为唯一权威；本文档不声明任何阶段的当前状态。
...
- 单次运行的目标峰值统一内存不超过约 12 GiB，为 macOS 和其他进程保留余量。
  （2026-08-16 修订：最终测试机确认为 M1 / 16 GiB 统一内存，原 24 GB 包络
  随之收缩；见阶段 11。）
...
通用视觉模型不属于首版必需依赖，也不得在默认流程中下载。（2026-08-16 修订：
`Qwen3-VL-8B` 调研报告由维护者手动移除——通用视觉模型在代码合同中无消费方，
已明确出范围；原「继续保留在 `research/`」的要求随之作废。）
...
以下只是开发前包络，正式数值由原型和历史运行数据替换（2026-08-16 修订：
最终测试机为 M1 / 16 GiB，下表为 32 GB 机器时代的旧区间，阶段 11 原型
将以实测替换；超限门槛同步改为 12 GiB）：
...
阶段状态的唯一权威记录是 `project-state.json`；本节是这组状态词汇的所有者
（参见 ADR 0042），其他文档引用本节而不复制第二份定义。

单个阶段（`phase_history[].status` 与 `phase_status`）按以下顺序推进：

- `implementation_planning`：阶段规格已获批准
  （规格文档状态 `approved_for_implementation_planning`），原子任务清单
  或实现尚未开始。
- `in_progress`：实现任务已获用户授权并正在执行。
- `completed`：退出门禁全部通过，完成报告与清单已记录。

`verification` 取值：

- `passed`：验证含真实环境成分。
- `passed_offline`：仅使用项目内合成夹具与受控适配器的离线验证。

项目整体（`overall_stage`）按以下顺序推进：

- `engineering_development`：开发期，只能进行工程验证。
- `real_world_testing`：开发完成，真实视频验收进行中。
- `production_validated`：全部正式分支完成真实验收并经用户明确确认。
...
### 阶段 5：可审计音频分析原型（`auditable_audio_analysis_prototype`，强制对齐、VAD 与说话人分离）
...
（2026-08-16 修订：本阶段由当日拷问决定从原「真实视频测试」中拆出并插入；
原阶段 11 顺延为阶段 12。依据：真实测试前存在一整段此前未编排的能力启用
工程——模型采集、推理依赖引入、真实引擎接线与逐能力原型。选型论证见
`research/phase-11-model-selection.md`。）

目标：

- 让六个既有模型能力（asr_primary、asr_review、forced_alignment、vad、
  diarization、ocr_primary）和一个新能力（text_semantics）从
  `model_acquisition_required` 变为可真实执行且有原型证据。

主要工作：

1. 逐模型下载计划与单独确认，固定 revision 和哈希，更新模型注册表。
2. 推理依赖一次性授权清单（全链免 torch），经 uv 锁定引入。
3. 定义 text_semantics 能力与真实文本适配器；受控离线适配器保留为测试路径。
4. 补齐 diarization 注册表候选（免凭证 sherpa-onnx 管线）。
5. 重模型阶段独立子进程执行（退出即归还内存，峰值内存入证据）。
6. 资源包络常量与 plan 估算改造为 12 GiB。
7. 用维护者确认的公开真实素材做逐能力原型：工程检查 + 中文/英文样例输出
   交维护者目检；首次处理真实媒体时 `media_processed` 如实翻 `true`。
8. yt-dlp 按本机现版本重探测注册；失败才提出升级并单独确认。

退出门禁：

- 全部七个能力在真实素材上跑通原型且样例经维护者确认。
- 模型注册表每个采集条目含 revision、哈希、大小、量化与授权记录。
- 任一原型阶段实测峰值内存不超过 12 GiB。
- 工程自动化测试全绿；未经授权不发生任何下载。

### 阶段 12：真实视频测试

（2026-08-16 修订：由原阶段 11 顺延重编号，内容不变。）
...
当前阶段、阶段状态与下一阶段以 `project-state.json` 为准。任何阶段进入
实现前，先形成可检查的规格说明和原子化任务清单，并向用户提交：
