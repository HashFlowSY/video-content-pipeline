...
- [ ] Detection and sampling consume controlled, hash-pinned synthetic
...
- [ ] The same input and rule versions always produce the same selected
  frames, the same pages, and the same appearance records; rule versions are
  recorded in provenance.
- [ ] Every extracted frame — selected for OCR or not — enters the Retained
...
- [ ] `visual_page_id` is scoped to exactly one Part (ADR 0048); Page
